...
> Status: 🟢 **Фазы 1–3 done** (фундамент + админка + конструктор + навигатор + личный кабинет). Далее — Фаза 4 (workflow, отдельным планом).
...
| **M4** ✅ | Фронт админки | `lib/api/poa.ts`; хаб; каталог (вкладки CRUD); реестр (+выдача оригинала); матрица (клик-ячейка); сотрудники (+resolve); заявки (approve/reject). На `@/components/ui` + токенах МГТС | юрист ведёт каталог/матрицу/реестр визуально |
...
- **Модуль 4 (Навигатор) ✅** — `services/poa/navigator.py` (ЕДИНЫЙ источник правил для веба и
  будущего LLM): пошаговый движок `step()` + `route()` (35 сценариев, приоритет блоков А→К).
  `POST /poa/navigator/step` (доступен роли employee). Фронт: `services/poa/navigator` —
  вопросы по одному, прогресс-бар `ProgressLinear`, мгновенный результат со ссылкой на заявку.
  Навигатор виден всем в хабе (самообслуживание). Выход = ссылка на BR/ServiceDesk (пока внешняя;
  преднастроенный вход в собственный workflow — Фаза 4).

### Phase 3 — Мои доверенности ✅
Модуль 5: тонкий view поверх реестра + статусов заявок. `api/poa/my.py`
(`GET /my/certificates`, `GET /my/requests`) — доступен роли employee. Фронт:
`services/poa/my` (свои доверенности + статусы заявок), карточка в хабе (всем).
**Связка сотрудник↔учётка пока по ФИО** (`User.full_name`), т.к. SSO нет —
заменить на устойчивый id при появлении SSO. Скачивание скана и запрос отзыва/
продления — с файловым хранилищем и workflow (Фаза 4).
